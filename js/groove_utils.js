...
class GrooveUtils {
    constructor(excludeAbcForTesting = false) {
        this.documentOnClickHanderCloseContextMenu = (event) => {
            if (this.visible_context_menu) {
                this.hideContextMenu(this.visible_context_menu);
            }
        };
        this.tempoUpdateFromTextField = (event) => {
            var newTempo = Number(event.target.value);
        ...
        this.tempoUpdateFromSlider = (event) => {
            this.tempoUpdate(Number(event.target.value));
        };
        this.repeatMIDI_playback = () => {
        ...
        this.swingUpdateEvent = (event) => {
            if (this.swingIsEnabled === false) {
                this.setSwingSlider(0);
            }
            else {
                this.swingUpdateText(Number(event.target.value));
                this.updateRangeSlider('swingInput' + this.grooveUtilsUniqueIndex);
            }
        };
        this.loadFullScreenGrooveScribe = () => {
            var fullURL = this.getUrlStringFromGrooveData ? this.getUrlStringFromGrooveData(this.data, 'fullGrooveScribe') : this.data.toUrl();
            var win = window.open(fullURL, '_blank');
            if (win) {
                win.focus();
            }
        };
        this.metronomeMiniMenuClick = () => {
    ...
    hideContextMenu(contextMenu) {
        ...
        const target = (contextMenu || this.visible_context_menu);
        if (target) {
    ...
    ;
    create_note_mapping_array_for_highlighting(HH_array, snare_array, kick_array, toms_array, num_notes, HH2_array) {
    ...
    tempoUpdate(tempo) {
        var textField = document.getElementById('tempoTextField' + this.grooveUtilsUniqueIndex);
        if (textField)
    ...
    ;
    MIDI_from_HH_Snare_Kick_Arrays(midiTrack, HH_Array, Snare_Array, Kick_Array, Toms_Array, midi_output_type, metronome_frequency, num_notes, num_notes_for_swing, swing_percentage, timeSig, HH2_Array) {
        ...
        MIDI_from_HH_Snare_Kick_Arrays(midiTrack, HH_Array, Snare_Array, Kick_Array, Toms_Array, midi_output_type, metronome_frequency, num_notes, num_notes_for_swing, swing_percentage, timeSig, this.metronomeSolo, offsetClickStartBeat, HH2_Array);
    }
    ;
    create_MIDIURLFromGrooveData(myGrooveData, MIDI_type) {
        ...
        for (var measureIndex = 0; measureIndex < myGrooveData.numberOfMeasures; measureIndex++) {
            ...
            this.note_mapping_array = this.note_mapping_array.concat(this.create_note_mapping_array_for_highlighting(hhArray, snareArray, kickArray, tomsArray, fullSizePerMeasure, hh2Array));
        }
    ...
    getMidiPlayTime() {
        ...
        if (TotalPlayTime) {
            ...
            var delta_time_diff = new Date(time_now.getTime() - global_last_midi_update_time);
    ...
    setTempo(newTempo) {
        if (newTempo < 19 && newTempo > 281)
            return;
        var input = document.getElementById("tempoInput" + this.grooveUtilsUniqueIndex);
        if (input)
...
//# sourceMappingURL=groove_utils.js.map